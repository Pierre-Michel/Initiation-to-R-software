...
L = LETTERS[1:3]
D = data.frame(x=rep(1,5), y=1:5, fac=sample(L, 5, repl=T))
D
```
...
# Try this example
library(cluster) 
data(agriculture) # or data(agriculture,package="cluster")
```
...
knitr::kable(data.frame(function_name = c("var(x) or cov(x)", "cor(x)", "sd(x)", "round(x,n)", "rev(x)", "sort(x)", "rank(x)", "scale(x)"),
...
knitr::kable(data.frame(function_name = c("pmin(x,y,...)","cumsum(x)","cumprod(), cummin(), cummax()","match(x,y)","choose(x,k)","na.omit()","na.fail()"),
                        description = c("A vector whose element i is the minimum between x[i] and y[i]",
                                        "A vector whose element i is the sum of x[i] and y[i]",
                                        "Similar as product, minimum, maximum",
                                        "Returns a vector of same length as x containing elments of x that are in y",
...
knitr::kable(data.frame(function_name = c("unique()","table()","subset()", "sample(x,n)"),
                        description = c("If x is a vector or data.frame, returns a similar object with duplicated observations removed",
                                        "Returns a table of counts of the different values of x",
                                        "Returns a selection of x based on criteria",
                                        "Random sampling, without replacement, of a sample of elements of x, of size n")))
```
