...
N <- c(1, 2, 3)
h = n + N
n = 5 ; M = matrix(1:10, 2, 5); N <- c(1, 2, 3); h = n + N
```
...
x = sample(c(1, 2, 3, 4), 3, prob = c(0.1, 0.2, 0.6, 0.1))
...
u = rnorm(100) # normal distribution with parameters 0 and 1
v = rnorm(100, 2, 3) # normal distribution with parameters 2 and 3
w = rpois(100, 3) # Poisson distribution with parameter 3
par(mfrow = c(1,3)); hist(u); hist(v); hist(w); # plot histograms
```
...
```{r fac_cut, echo=T, eval=T, warning=F}
